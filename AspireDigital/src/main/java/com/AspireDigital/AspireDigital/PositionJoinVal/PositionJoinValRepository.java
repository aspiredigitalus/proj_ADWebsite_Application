...
public interface PositionJoinValRepository extends JpaRepository<PositionJoinVal, Integer> {
    ...
    @Query(value="SELECT p.id, p.employee_id, p.position_id, v.position_title FROM position p JOIN position_val v ON p.position_id = v.position_id WHERE employee_id = :employee_id", nativeQuery = true)
